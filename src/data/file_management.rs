        ...
        for value in to_do_list_text {
            for value_second in value {
                ...
                match word_separated_by_colon[0].as_str() {
                    ...
                    "description" => {
                        task.description = word_separated_by_colon[1].trim().to_owned().clone()
                    }
                    "status" => {
                        task.status_progress = word_separated_by_colon[1].trim().to_owned().clone()
                    }
                    "createdAt" => {
                        task.created_at = word_separated_by_colon[1].trim().to_owned().clone()
                    }
                    "updatedAt" => {
...
pub fn task_list_format(task_list: Vec<RegisterTask>) -> String {
    ...
    for (index, task) in task_list.iter().enumerate() {
        ...
        match &task.updated_at {
            Some(value) => updated_at.push_str(value),
            None => updated_at.push_str("None"),
        }
        ...
        if index == task_list.len() - 1 {
            format_list_json.push_str(&format!(
                ...
                ",
                task.id, task.description, task.status_progress, task.created_at, updated_at
            ));
        } else {
            format_list_json.push_str(&format!(
                "

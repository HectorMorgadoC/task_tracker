mod interface;
use std::{fs::{File,OpenOptions}, io::{Read,prelude::*}};

use crate::interface::task_inteface::RegisterTask;

fn main() {

    println!("{:?}", task_log(convert_to_do_lists(read_file("src/data.json"))));
...
fn read_file(path: &str) -> String {
    ...
    match data_task {
        Ok(mut value) => {
            value.read_to_string(&mut task_list).unwrap_or(0);
            task_list
        }
...
fn convert_to_do_lists(file_reading: String) -> Vec<Vec<String>> {
...
fn task_log(to_do_list_text: Vec<Vec<String>>) -> Vec<RegisterTask> {
